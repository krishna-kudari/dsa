...
public class Representation {
    ...
    public static void solve() throws IOException {
        ...
        for (int[] edge : edges) {
            ArrayList<Edge> nbrListA = adjList.get(edge[0]);
            ArrayList<Edge> nbrListB = adjList.get(edge[1]);
            nbrListA.add(new Edge(edge[0], edge[1], 0));
            nbrListB.add(new Edge(edge[1], edge[0], 0));
        }
        for (ArrayList<Edge> nbrs : adjList) {
            for (Edge e : nbrs) {
                System.out.print(e + ", ");
            }
            System.out.println();
        ...
        // Adjacency set (useful for O(1) edge lookup)
        ArrayList<HashSet<Integer>> adjSet = new ArrayList<>();
        for (int i = 0; i < 5; i++) {
        ...
        System.out.println("\nAdjacency Set:");
        for (int i = 0; i < 5; i++) {
            System.out.println(i + " -> " + adjSet.get(i));
        }
        ...
        int[] rowPtr = new int[numNodes + 1];  // size = numNodes + 1
        int[] colIdx = new int[numEdges * 2];  // size = 2 * numEdges (undirected)
        ...
        // Build rowPtr (cumulative sum of degrees)
        rowPtr[0] = 0;
        ...
        // Fill colIdx
        int[] currentPos = rowPtr.clone();
        for (int[] edge : edges) {
            colIdx[currentPos[edge[0]]++] = edge[1];
            colIdx[currentPos[edge[1]]++] = edge[0];
        }

        System.out.println("\nCompressed Sparse Row (CSR):");
        System.out.println("rowPtr: " + Arrays.toString(rowPtr));
        System.out.println("colIdx: " + Arrays.toString(colIdx));
        System.out.println("To get neighbors of node i: colIdx[rowPtr[i]] to colIdx[rowPtr[i+1]-1]");

        // Incidence matrix (rows = nodes, cols = edges)
        // For undirected: entry is 1 if node is incident to edge, 0 otherwise
        int[][] incidenceMatrix = new int[numNodes][numEdges];
        for (int e = 0; e < numEdges; e++) {
            incidenceMatrix[edges[e][0]][e] = 1;
            incidenceMatrix[edges[e][1]][e] = 1;
        }
        System.out.println("\nIncidence Matrix (rows=nodes, cols=edges):");
        System.out.println("Edges: " + Arrays.deepToString(edges));
        for (int i = 0; i < numNodes; i++) {
            System.out.println("Node " + i + ": " + Arrays.toString(incidenceMatrix[i]));
        }
    }
